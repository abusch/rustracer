...
pub fn parse<I: Stream<Item = Tokens>, A: Api>(
    ...
    let object_begin = (token(Tokens::OBJECTBEGIN), string_())
        .and_then(|(_, name)| api.object_begin(name).map_err(|e| e.compat()));
    let object_end =
    ...
    let object_instance = (token(Tokens::OBJECTINSTANCE), string_())
        .and_then(|(_, name)| api.object_instance(name).map_err(|e| e.compat()));
    let world_begin =
    ...
    let rotate = (token(Tokens::ROTATE), num(), num(), num(), num())
        .and_then(|(_, angle, dx, dy, dz)| api.rotate(angle, dx, dy, dz).map_err(|e| e.compat()));
...
fn param_list<'a, I: Stream<Item = Tokens> + 'a>() -> Box<Parser<Input = I, Output = ParamSet> + 'a>
{
...
fn param_type<'a, I: Stream<Item = char> + 'a>() -> Box<Parser<Input = I, Output = ParamType> + 'a>
{
    choice!(
...
fn num_array<'a, I: Stream<Item = Tokens> + 'a>() -> Box<Parser<Input = I, Output = Vec<f32>> + 'a>
{
    choice!(
        try(between(
            token(Tokens::LBRACK),
